...
- [ ] **Step 1: 创建 asr 目录和 **init**.py，确保 .gitignore 包含 **pycache\*\*\*\*
...
export default defineEventHandler(async (event) => {
  ...
  const success = startASR({ url, provider, model: model || '' }, source, streamUrl)
  ...
  <div class="panel">
    ...
    <div class="panel-content">
      ...
      <div class="action-row">
        ...
        <button v-else class="action-btn stop" :disabled="isLoading" @click="emit('stop')">
          STOP ASR
        </button>
      </div>
    </div>
